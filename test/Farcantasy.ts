...
describe('Farcantasy contract tests', () => {
  ...
  describe('Constructor', function () {
    it('should deploy the contract with the correct fields', async function () {
      ...
      const version = '1.0.0'
      const contract = await this.factory.deploy(name, symbol, version)
  ...
  it('should correctly set idCap and mint', async function () {
    const version = '1.0.0'
    const contract = await this.factory.deploy('Farcantasy', 'FRCNTSY', '1.0.0')
    expect(await contract.idCap()).to.equal(1000)
    // Call setIdCap
    await contract.setIdCap(1)
    // Mint
    await contract.mint({ value: utils.parseEther('10') })
    await expect(
      contract.mint({ value: utils.parseEther('10') })
